...
public partial class Admin_frmAdvatizmentReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                Display_List(gvUserList, "SELECT  [Register_Id],[Name],[Mobile_No],[EmailId],[Address] ,convert(varchar(12),DOB,103)as [DOB],'../User_Photos/'+Photo as Photo,Isactive,(Select Username from tbl_user TU where TU.Staff_Id=U.Register_Id)as Username,(Select Password from tbl_user TU where TU.Staff_Id=U.Register_Id)as Password FROM tbl_Register_User U where  Isactive='true' Order by Register_Id desc");
            }
        }
        catch (Exception Ex)
        {
    ...
    //        //    AddColum = AddColum + " CreatedById in (0" + CoveredAreaIds + "0) and ";
            ...
            if (AddColum != "")
            {
                ...
                Display_List(gvUserList, "SELECT  [Register_Id],[Name],[Mobile_No],[EmailId],[Address] ,convert(varchar(12),DOB,103)as [DOB],'../User_Photos/'+Photo as Photo,Isactive,(Select Username from tbl_user TU where TU.Staff_Id=U.Register_Id)as Username,(Select Password from tbl_user TU where TU.Staff_Id=U.Register_Id)as Password FROM tbl_Register_User U where  Isactive='true' and "+AddColum+" Order by Register_Id desc");
            }
            else
            {
                Display_List(gvUserList, "SELECT  [Register_Id],[Name],[Mobile_No],[EmailId],[Address] ,convert(varchar(12),DOB,103)as [DOB],'../User_Photos/'+Photo as Photo,Isactive,(Select Username from tbl_user TU where TU.Staff_Id=U.Register_Id)as Username,(Select Password from tbl_user TU where TU.Staff_Id=U.Register_Id)as Password FROM tbl_Register_User U where  Isactive='true' Order by Register_Id desc");
