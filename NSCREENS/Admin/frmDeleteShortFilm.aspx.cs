...
public partial class Admin_frmDeleteShortFilm : System.Web.UI.Page
{
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataSet objDataSet = MasterCode.RetrieveQuery("Update tbl_Language_Short_FilmId set Isactive='false' where Lan_Short_film_Id=" + lnk.CommandName);

            ShowNotification("Short Film", "Short Film Deleted Successfully..!", NotificationType.success);
            ...
            if (lblDumpEmail.Text != "")
            {
                using (StringWriter sw = new StringWriter())
                {
                    using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                    {
                        ...
                        ShowNotification("Short Film", "Mail Sent successfully..!", NotificationType.success);
                    }
                }
            }
            else
                ShowNotification("Short Film", "This Video upload by Admin..!", NotificationType.error);
        }
        catch (Exception Ex)
        {
